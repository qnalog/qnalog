...
原先首页是「使用准备」四张卡片（纪要转写 / AI 整理 / 电脑音频 / Obsidian 日记）。
它把两类不同的东西混在一起，而且回答不了用户最常问的那个问题：

| 问题 | 原「使用准备」 | 现「程序状态」 |
|---|---|---|
| 我现在能用了吗？ | 需要自己把四张卡的状态拼起来看 | 一句话结论：「已配置妥当，可以开始使用」 |
| 现在实际在跑哪个模型？ | 不显示，要去 API 页翻 | 直接列出三项模型 |
| 电脑音频、日记属于这里吗？ | 混在同一组卡片里 | 移出。它们不是「是否就绪」的一部分，设置项仍在各自页面 |
| 想换服务去哪？ | 卡片上的按钮跳到各自页面 | 一个「调整配置」按钮，跳 API 页 |

明细固定四项，用户形成固定阅读位置；每行显示**英文模型名**，不带中文供应商标识
（供应商由上面的服务选择决定，混在模型名里既长又容易与模型改名脱节）：

```
语音转写    qwen-audio-3.0-asr-flash-streaming
AI 整理     qwen3.8-flash
说话人识别   qwen-audio-3.0-asr-flash-filetrans
音频输入    仅麦克风
```

**实现方式**：用 Obsidian 原生的 `Setting` 行（`setName` + `setDesc` + `addButton`），
与下方「进阶能力」区块同一套结构。没有为这块新增自定义 CSS 类：

- 标题与内容的间距、字号、颜色由原生样式与 `.qnalog-home-block` 提供，
  与页面其他部分一致，不会各自漂移。
- 原先「使用准备」卡片用的 `.qnalog-home-prep-grid` / `-prep` / `-prep-name` /
  `-prep-meta` / `-prep-actions` / `-home-chip` 已随卡片一起删除（保留仍在用的
  `.qnalog-home-prep-desc`，快速配置面板还用）。
- 不建议再为首页另起一套状态样式：两套样式会导致同一类信息在两处长得不一样。

**判定口径**：总览只看「是否缺配置」，不看测试结果。测试结果属于 §10.3 的四态，
由各服务自己的徽章承担。两者混在一起会让总览在用户改一个字符时就翻脸，
而且与徽章职责重复。

说话人识别只有声明了该能力的服务才提模型；没声明就如实说「当前导入音频服务不做说话人识别」，
不留空行也不假装可用。

**配好与没配好时的说明文字不同**（两者都经用户确认）：

| 情形 | 明细行显示 | 说明文字 |
|---|---|---|
| 已配好 | 三项模型 | 以下为当前正在使用的模型，可点击调整配置按钮进行修改。 |
| 缺配置 | 缺什么（访问密钥未填写 / 模型名称未填写） | 下面标出的项目还缺内容，补齐后即可开始使用；也可以用「快速配置」一次填好。 |

缺配置时不能说「以下为当前正在使用的模型」——下面列的是缺什么，与行内容自相矛盾。

数据来自 `buildSetupStatus`（`src/setup/index.ts`，有类型检查），
由 `tests/bailian-setup.test.ts` 的 6 项覆盖（含反向验证：让它永远宣称可用、
或让它不显示模型，对应用例都会失败）。
