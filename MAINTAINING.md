...
界面上的体现：首页「使用准备」卡片与服务页徽章都用这四态，不再显示含糊的「已配置 / 已填写」。
...
```
语音转写    阿里云百炼实时转写 · qwen-audio-3.0-asr-flash-streaming
AI 整理     qwen3.8-flash
说话人识别   阿里云百炼录音文件识别 · qwen-audio-3.0-asr-flash-filetrans
音频输入    仅麦克风
```
...
由 `tests/bailian-setup.test.ts` 的 4 项覆盖（含反向验证：让它永远宣称可用、
或让它不显示模型，对应用例都会失败）。
