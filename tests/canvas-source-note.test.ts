...
describe("语义图源纪要解析", () => {
  it("优先使用 Canvas 内嵌的源纪要路径", () => {
    expect(resolveSemanticCanvasSourcePath({
      qnalogSemantic: { sourcePath: "QnALog\\转写纪要\\会议.md" },
    }, "错误 · 语义图.canvas")).toBe("QnALog/转写纪要/会议.md");
  });

  it("旧语义图没有元数据时可从同名文件回退", () => {
    expect(inferSemanticCanvasSourcePath("QnALog/转写纪要/会议 · 语义图.canvas"))
      .toBe("QnALog/转写纪要/会议.md");
    expect(inferSemanticCanvasSourcePath("QnALog/转写纪要/会议 · 语义图（紧凑预览）.canvas"))
      .toBe("QnALog/转写纪要/会议.md");
  });
