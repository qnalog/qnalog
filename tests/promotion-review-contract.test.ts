...
describe("promotion review runtime contract", () => {
  it("正式注册模式、frontmatter 和最终提示词", () => {
    ...
    expect(pluginSource).toContain('"promotion-review": buildPrompt(MODE_BODIES["promotion-review"]');
    expect(pluginSource).toContain('"promotion-review": ["主题", "被评审人", "岗位", "当前职级", "目标职级"');
  ...
  it("录音会话冻结上下文，并将初审写入笔记和最终原始材料", () => {
    expect(pluginSource).toContain('promotionReviewContext: this._currentPromotionReviewContext || null');
    expect(pluginSource).toContain('promotionReviewPhase: "presentation"');
    expect(pluginSource).toContain('lexvoice-promotion-pre-review-start');
    expect(pluginSource).toContain('const promotionPreReviewBlock = buildPromotionPreReviewDetails(session)');
    expect(pluginSource).toContain('promotionReviewContext: session.promotionReviewContext || null');
  });

  it("长答辩分部提取证据后统一生成一份全局报告", () => {
    expect(pluginSource).toContain("buildPromotionReviewPartContextPrefix");
    expect(pluginSource).toContain("buildPromotionReviewPartInstruction");
    expect(pluginSource).toContain("buildPromotionReviewConsolidationPrompt");
    expect(pluginSource).toContain('(mode === "synthesis" || mode === "promotion-review") && partPlans.length > 1');
    expect(pluginSource).toContain('purpose: mode === "promotion-review" ? "promotion-review-consolidation"');
  });

  it("述职阶段走普通大纲，问答阶段只保留评委和候选人对话", () => {
    expect(pluginSource).toContain('detectPromotionReviewPhase(session.promotionReviewPhase, windowed.newSegments)');
    expect(pluginSource).toContain('modeKey === "promotion-review" && opts.promotionQa');
    expect(pluginSource).toContain('不要输出 XML、JSON、Markdown、图标、编号、AI评价、建议追问');
    expect(pluginSource).toContain('if (isPromotionReview) return;');
    expect(pluginSource).toContain('isPromotionReview ? "评委" : "面试官"');
  });
