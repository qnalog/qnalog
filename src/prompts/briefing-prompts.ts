...
${frontmatterSection}**整体结构原则**：顶部用 callout 做结构化速览（摘要、必要时的决策清单/录用建议），**主体内容贴近原文按实际推进顺序展开**——用二级标题（\`##\`）分节 + 散文段落叙述，需要再分层时才用三级标题（\`###\`），不强行套"讨论要点 / 分歧 / 暂行结论"等模板框。关键判断引用用普通 \`> \` blockquote 即可，不要为每个话题再套 callout。

**标题层级与编号**：一级标题（\`#\`）是文档标题，由插件写入，**正文不要输出 \`#\`**；章节一律用 \`##\`，子节用 \`###\`，不跳级；章节标题**不加**"1. / 一、"这类编号——阅读顺序由大纲面板体现，手工编号在后续编辑中会失准。
...
- callout 最后一行必须是实际内容，**不要用空的 \`>\` 行收尾**；callout 结束后接标题、列表或正文前，先空一个普通空行
...
export function buildAdaptiveBriefingLengthInstruction(mode, stats) {
  ...
  if (mode === "synthesis") {
    const lines = [
      ...
      "- 开头先写全场梗概；正文按真实成立的主要议题分节展开（每节一个二级标题、不加编号），通常为 3–6 个，简单材料可以更少，复杂长会可以更多。章节划分代表讨论结构，不代表内部处理分段。",
  ...
  if (isUltraLong) {
    ...
    lines.push("- 每个被实际讨论过的主题，至少展开成一段完整叙述（背景 → 展开 → 结论或分歧），不要把一个详细讨论过的主题压成单句。一小时以上的会议，主体通常应有多个二级标题章节、整体篇幅明显长于短会纪要。");
