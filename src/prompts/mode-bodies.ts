...
2. 再识别真正成立的主要议题，归并为章节。通常为 3–6 个；材料简单可以更少，复杂长会可以更多。章节是阅读结构，不是机械凑数。
...
按访谈实际推进顺序展开，**覆盖全程每一个被实际讨论过的话题，不跳过、不合并掉任何有内容的话题段**。每个自然话题用二级标题（6–12 字概括话题主旨）。长访谈（多话题、高信息密度）的主体应充分展开，不要为求简短而把多个话题挤进一两段。
...
按会议实际推进顺序展开（不预设议程），**覆盖每一个被实际讨论过的议题，不跳过、不把多个议题压成一句概述**。每个议题用二级标题（6–12 字）。长会议或多议题会议的主体应随议题数量充分扩展，「更结构化」指层次更清晰，不等于「更简略」。
