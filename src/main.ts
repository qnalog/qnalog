...
import {isRecord, getErrorMessage, pickDefined, genId, pad, formatElapsed, sanitizeFilename, escapeRegExp } from "./shared/util-common";
...
import {VIEW_TYPE_OUTLINE, clipRealtimeContextText, hasRealtimeOutlineRunnableBacklog, normalizeRealtimeOutlineState } from "./notes/realtime-outline";
...
class LexVoicePlugin extends obsidian.Plugin {
  ...
  canRunMeetingWorkbenchInteraction(session, opts = {}) {
    ...
    const outlineState = this.outline.getRealtimeOutlineCoordinatorState();
    if (
  ...
  async processPendingMeetingWorkbenchInteractions(session, opts = {}) {
    ...
    try {
      ...
      this.outline.ensureRealtimeOutlineProgress(session, "workbench-finished");
    }
  }
