    ...
    startBtn.onclick = async () => {
      ...
      if (flow === "settings" || flow === "recording") {
        ...
        try {
          ...
          console.error("[QnALog] create interview brief from settings failed", e);
...
export class ImportTextModal extends obsidian.Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl("p", { cls: "lexvoice-import-desc" })
      .setText("选择已有 Markdown、速录稿或文本纪要。QnALog 不会调用语音转写服务，会直接走 API 页的「AI 整理服务」LLM 链路并按当前模板结构化整理。选择招聘评估时，会先弹出 JD / 简历 / 候选人信息窗口。");
