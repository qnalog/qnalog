...
export const QNALOG_UPDATE_REPO_URL = "https://github.com/qnalog/qnalog";
export const QNALOG_UPDATE_BRANCH = "main";
export const QNALOG_UPDATE_PLUGIN_DIR = "";
export const QNALOG_UPDATE_RAW_BASE_URL = "";
...
export function resolveUpdateRawBase(_settings?: unknown): string {
  const rawBase = QNALOG_UPDATE_RAW_BASE_URL.trim().replace(/\/+$/g, "");
  if (rawBase) return rawBase;
  const repo = parseGithubRepoUrl(QNALOG_UPDATE_REPO_URL);
  if (!repo) return "";
  const branch = QNALOG_UPDATE_BRANCH.trim() || "main";
  const subdir = trimSlashes(QNALOG_UPDATE_PLUGIN_DIR);
...
export function resolveUpdateRawBases(settings?: unknown): string[] {
  ...
  if (QNALOG_UPDATE_RAW_BASE_URL.trim()) return out;
...
export function isTrustedUpdateSourceUrl(url: string, repoUrl: string = QNALOG_UPDATE_REPO_URL): boolean {
