...
export class TranscribeProfileService {
  ...
  getTranscribeProviderProfile(id, provider) {
    const profiles = {
      ...
      "dashscope-chat": {
        title: "Alibaba Cloud Bailian Qwen3-ASR Flash",
        badge: "Cloud Transcription",
        transcribeMode: "segmented",
        requiresKey: true,
        ...
        languagePlaceholder: "",
        endpointHelp: "Bailian's OpenAI-compatible URL. Keep the default; Q&A Log appends /chat/completions itself.",
        keyHelp: "Enter the access key (API Key) created in the Bailian console. The key is stored obfuscated (not encrypted) in this vault's plugin settings file and is never uploaded.",
        modelHelp: "qwen3-asr-flash is recommended. It is a non-realtime HTTP model, so it works on both desktop and mobile. For speaker diarization use \"Alibaba Cloud Bailian Recording File Recognition\" under speaker recognition instead.",
        description: "Transcription over Bailian's HTTP interface. Q&A Log uploads audio in segments according to the configured segment interval. Unlike real-time transcription, this works on mobile as well: mobile cannot attach the authentication header that WebSocket real-time recognition requires.",
        priceHint: "Billed according to Bailian audio recognition; the Bailian console price is the final authority. Each request accepts up to 5 minutes and 10MB.",
        steps: ["Create an API Key in the Bailian console", "Keep the default service URL and model name", "Just start recording"],
        links: [
          ["Access Key", "https://help.aliyun.com/zh/model-studio/get-api-key"],
