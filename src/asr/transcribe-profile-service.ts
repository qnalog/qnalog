  ...
  getTranscribeProviderProfile(id, provider) {
    const profiles = {
      siliconflow: {
        ...
        description: "OpenAI 兼容的音频转写接口。Q&A Log 会按设定的分段间隔切段上传。",
      ...
      openrouter: {
        ...
        languagePlaceholder: "",
        endpointHelp: "OpenRouter 的语音转写接口，通常保持默认即可。",
        keyHelp: "填写 OpenRouter 控制台创建的 API Key。",
        modelHelp: "填写 OpenRouter 上的转写模型标识，例如 openai/whisper-large-v3。可在控制台按 output_modalities=transcription 筛选可用模型。",
        description: "OpenRouter 提供 OpenAI 兼容的转写接口。Q&A Log 按设定的分段间隔切段上传。",
        priceHint: "按音频时长与所选模型计费，以 OpenRouter 控制台的用量页为准。",
        steps: ["注册或登录 OpenRouter 账号", "在控制台创建 API Key", "确认模型标识后运行连通性测试"],
        links: [
          ["访问密钥", "https://openrouter.ai/settings/keys"],
          ["转写文档", "https://openrouter.ai/docs/guides/overview/multimodal/stt"],
        ],
        note: "单次上传上限 25 MB；超长录音按分段间隔切段后上传，不受此限制。",
      },
      "dashscope-filetrans": {
        title: "阿里云百炼录音文件识别",
        badge: "长音频 · 说话人分离",
        ...
        modelHelp: "推荐 qwen-audio-3.0-asr-flash-filetrans（支持说话人分离）。fun-asr / paraformer-v2 仍可使用。",
        description: "导入音频专用。Q&A Log 直接提交整段音频，不在本地切成多个 ASR 任务。",
      ...
      apimimo: {
        ...
        endpointHelp: "小米 MiMo 的服务地址，保持默认即可。Q&A Log 会按 MiMo 要求的专用格式发送音频，与其他转写服务不同，无需手动调整。",
        keyHelp: "填写小米 MiMo 平台的访问密钥（API Key）。密钥以混淆（非加密）形式保存在本库的插件设置文件中，不会上传。",
        modelHelp: "固定使用 mimo-v2.5-asr。该服务只接受 wav/mp3、单段约 7.5MB 以内的音频；其他格式或更长的录音会由 Q&A Log 自动转换、切段后上传，无需手动处理。",
        description: "APIMiMo-V2.5-ASR 通过 OpenAI 兼容 Chat Completions 的 input_audio 识别音频。服务端只收 wav/mp3：选用本服务时 Q&A Log 会以 WebM/Opus 录音并在本机转成 WAV 分块上传；wav/mp3 文件未超限则直接发送。",
        priceHint: "按 MiMo 平台计费。大小限制由 Q&A Log 自动处理：超限录音会按约 3 分钟自动切块上传，无需手动干预。",
        ...
        note: "此服务不支持热词参数；Q&A Log 会在转写结果返回后做本地热词纠错。在上方「识别语言」填 zh 或 en 可提升准确率。注意：此前用其他服务录制的 m4a/mp4 录音无法用本服务重新转写（仅影响重转写，新录音不受影响），如需重转写请临时切回原服务。",
      },
      "openai-realtime": {
        ...
        description: "流式转写，边说边出文字。Q&A Log 跳过分段切片，整场录音与服务保持一条实时连线，延迟约半秒以内。",
      ...
      "openai-realtime-translate": {
        ...
        description: "流式语音翻译。自动检测说话者语言，实时输出译文+原文双轨笔记。模型同时返回译音流（Q&A Log 自动丢弃，仅保留文字）。",
      ...
      dashscope: {
        ...
        languagePlaceholder: "",
        endpointHelp: "百炼实时识别的 WebSocket 地址。保持默认即可。",
        keyHelp: "填写百炼控制台创建的访问密钥（API Key）。密钥以混淆（非加密）形式保存在本库的插件设置文件中，不会上传。",
        modelHelp: "推荐 qwen-audio-3.0-asr-flash-streaming（多语种、支持方言）。旧配置的 paraformer-realtime-v2 仍可使用。",
        description: "流式转写，边说边出文字。Q&A Log 跳过分段切片，整场录音与服务保持一条实时连线。",
        priceHint: "按百炼实时识别计费，以百炼控制台价格为准。",
        steps: ["在百炼控制台创建 API Key", "保持默认服务地址和模型名", "开始录音即可"],
        links: [
          ["访问密钥", "https://help.aliyun.com/zh/model-studio/get-api-key"],
          ["实时语音识别文档", "https://help.aliyun.com/zh/model-studio/real-time-speech-recognition-user-guide"],
        ],
        note: "流式模式下「分段间隔」「即时分段」设置不生效；笔记会在录音过程中实时追加文字。桌面端使用流式连接；移动端若要实时转写，请在「进阶」里改用分段转写服务。",
      },
      local: {
        title: "本地转写服务",
        badge: "本地服务",
        ...
        description: "适合隐私优先或离线工作流。Q&A Log 不负责下载模型或启动服务，只负责把音频发送到已启动的本地转写服务。",
      ...
      whisperx: {
        ...
        note: "Q&A Log 只负责调用已启动的服务，不负责安装模型。为保持说话人编号一致，录音停止后统一转写。",
      },
      custom: {
        title: "其他转写服务",
        badge: "高级",
